...
The identity $B(F)k^n$ is \cref{prop:base} together with inclusion-exclusion over $\{2,3\}$, so the left column is Proved; the table is the finite domain on which it was re-run to the integer. The design is spanning with $k=8>6$, so \cref{thm:main} applies and its density is
\[
...
\label{cor:equi}
Let the design be spanning, let $d$ satisfy $\gcd(d,q)=1$, and let $a \in (\mathbb{Z}/d)^{D}$. Then
\[

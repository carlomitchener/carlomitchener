...
\begin{tikzpicture}[x=12cm,y=1cm]
  \draw[line width=3pt,black!25] (0,0) -- (0.446717,0);
  \draw[line width=3pt,black!25] (0.640212,0) -- (1,0);
  \draw[line width=3pt,red!65!black] (0.446717,0) -- (0.640212,0);
  \draw[->] (0,0) -- (1.03,0);
  \foreach \x/\l in {0/0, 1/1} {
    \draw (\x,-0.09) -- (\x,0.09) node[above,font=\footnotesize] {$\l$};
  }
  \draw (0.446717,-0.12) -- (0.446717,0.12);
  \draw (0.640212,-0.12) -- (0.640212,0.12);
  \node[font=\footnotesize,align=center,anchor=north] at (0.446717,-0.18) {$0.446717$\\[-1pt]\scriptsize moment ladder};
...
\end{tikzpicture}
\caption{The pincer. Grey is proved closed, red is what remains, and the upper red endpoint is this paper's theorem. The dashed marks are structural walls: no moment order reaches $0.447931$, no per-ray maximum reaches $1/2$.}
\label{fig:pincer}
\end{figure}

Above dimension one this is a routine sieve, and the reason is worth seeing, because it is exactly the reason the sieve stops at a line. A prime $p$ in the greatest common divisor confines the point to the sublattice $p\mathbb Z^2$, whose share of a $D$-dimensional digital design is of order $p^{-D}$; summing over the large primes costs $\sum_{p > z} p^{-D}$, which converges for $D > 1$ and can be made small by taking $z$ large. At $D = 1$ that sum is $\sum_{p>z} 1/p$, which diverges for every $z$, and the argument stops. The gasket is the canonical dimension-one object. Nothing below depends on the picture just drawn; it is only the reason dimension one is the interesting case. What blocks the proof there is a single estimate, called Lemma B below: the points of $G_n$ whose coordinates a large prime $p$ divides must not be too many, summed over all large $p$ at once.
...
\[
  \beta = \frac{\log_3 p}{n}.
\]
Two completely different machines close the two ends. From below, a Fourier moment ladder closes every $\beta$ under $0.446717310462$; that machine is stated here as \cref{def:hypL} and is not reproduced, because it shares nothing with this paper. From above, and this is what we prove, an elementary automaton argument with no Fourier analysis in it at all closes every $\beta$ above
\[
...
\begin{definition}[Lemma B and its exponent]
\label{def:lemmab}
Fix a prime $p \ne 3$ and let
\[
  T_p(n) \ = \ \#\{\,x \in G_n \setminus \{0\} \ : \ p \mid \gcd(x_1,x_2)\,\},
\]
the mass a single prime carries in the greatest common divisor. The origin is excluded because it lies in every residue class of every modulus and carries no information. \emph{Lemma B} is the statement
\[
  \lim_{z \to \infty} \ \limsup_{n \to \infty} \ \frac{1}{3^n} \sum_{p > z} T_p(n) \ = \ 0 ,
\]
and \emph{Lemma B holds on an exponent range $I \subseteq (0,1]$} when the same limit taken over the primes with $\log_3 p / n \in I$ is $0$. Both sums are finite: a nonzero point of $G_n$ has both coordinates below $3^n$ and not both zero, so its gcd is a positive integer below $3^n$ and only primes $p < 3^n$ occur at all.
\end{definition}

\begin{definition}[Hypothesis L, the low range]
\label{def:hypL}
\emph{Hypothesis L} is the statement that Lemma B holds on $(0,\ \beta_0]$ for $\beta_0 = 0.446717310462$. It is a theorem of a separate machine, the Fourier moment ladder: exact moment identities for the gasket's character sums, H\"older across digit blocks, and a dyadic summation. It works with the target-uniform count $\max_\tau \#\{x \in G_n : x \equiv \tau \bmod p\}$, which dominates $T_p(n)$, so what it delivers on $(0,\beta_0]$ is at least \cref{def:lemmab}; that weaker consequence is all we import. The argument shares no object with this paper and is not reproduced here, so every statement below that uses it is labelled as using it.

One further statement is imported from the same machine and used nowhere in a proof, only as a signpost: its rungs $\beta_0^{(2K)}$ obey the cap $\beta_0^{(2K)} < 2/(3 + \log_3 5)$ for every $K$, because the Fourier peak forces the ladder parameter $\kappa_{2K}$ below $2$. So no moment order reaches $1/2$. The numbers this definition contributes are re-checked arithmetically in \cref{fact:ladder}.
\end{definition}
...
\]
Since $\psi_\varphi(c) < 1$ exactly when $c < c^* = (1 - \log_3\varphi)/(2 - \log_3\varphi) = 0.3597878$, Lemma B holds on $(\beta', 1]$ for every
\[
...
\label{cor:pincer}
Under \cref{def:hypL}, Lemma B at dimension one can fail only for prime exponents in
\[
  \beta \ \in \ (\,0.446717310462,\ \ 0.6402121938\,] .
\]
The upper endpoint is \cref{thm:topedge} and is proved here in full; the lower endpoint is Hypothesis L and is proved elsewhere by the moment ladder, not in this paper.
\end{corollary}
...
\begin{fact}[The three ladder numbers]
\label{fact:ladder}
...
and the order-$10$ growth constant $\lambda_{10} = 6664.113662506$ gives $\beta_0^{(10)} = 0.447597813454$.
The cap constant imported in \cref{def:hypL} evaluates to $2/(3 + \log_3 5) = 0.447930988$.
The order-$8$ rung is the one carrying a written three-block master bound, and it is the rung Hypothesis L uses. The order-$10$ rung is a rigorously computed ladder parameter whose three-block master bound is not written, so it is a strongly supported improvement and not a re-proof of target-uniform Lemma B at that edge. This Fact asserts only the arithmetic of the three displayed numbers, not the ladder theorems behind them; that arithmetic is checked by \texttt{scripts/verify.py}.
\end{fact}
...
$\rho(a,b) < \varphi$ for every non-shift ray, with $\sup \rho = 1.4655712319$, the real root of $x^3 = x^2 + 1$ \cite{oeissuper}. Under it, \cref{thm:topedge} improves to every $\beta' > 1/(2 - \log_3 1.4655712319) = 0.6053028664$.
...
\label{con:z}
A second moment over ray pairs, $Z(n) = \sum_y M_n(y)^2$, is conjectured to satisfy $Z(n) = 2 \cdot 3^n + o(3^n)$; under it the window of \cref{cor:pincer} shrinks to $(0.446717310462,\ 1/2]$, and the per-ray wall at $1/2$ of \cref{fig:pincer} is then the binding one.
...
\]
Two admissible digits at a $2$-branching state are distinct mod $3$ by \cref{lem:types}, and $3 \nmid a'$, so $a'd_i$ separates them: the two types at step $i+k$ differ. That is (2).
...
Fix a start state $s$ and $w \ge 1$. To an admissible path $(d_0,\dots,d_{w-1})$ assign
\[
  S \ = \ \bigl\{\, i + 1 \ : \ 0 \le i < w, \ \text{step } i \text{ is } 2\text{-branching},
  \ d_i \text{ designated} \,\bigr\} \ \subseteq \ \{1,\dots,w\} .
\]
...
Finally, Lemma B on $(\beta',1]$ follows. Let $p$ be a prime with $\log_3 p / n \in (\beta',1]$, so $p > 3^{\beta' n}$. Every point counted by $T_p(n)$ of \cref{def:lemmab} is a nonzero point of $G_n$ whose gcd is divisible by such a prime, hence is one of the $N_{\mathrm{top}}(n)$ points just bounded. A single point is counted for at most $1/\beta'$ of these primes, since their product divides its gcd and that is below $3^n$. Therefore
\[
  \sum_{\log_3 p / n \, \in \, (\beta',\,1]} T_p(n) \ \le \ \frac{1}{\beta'}\, N_{\mathrm{top}}(n) \ = \ o(3^n) ,
\]
so the inner $\limsup$ of \cref{def:lemmab} restricted to this range is $0$ for every $z$, and so is the limit.
\end{proof}
...
The first inequality is Jensen applied to the convex function $t \mapsto t^w$ on $[0,\infty)$; the second is monotonicity, $\int \rho^w d\nu \ge \nu(\{r_0\})\rho(r_0)^w = q\varphi^w$. Taking $w$-th roots, $(\int\rho^w d\nu)^{1/w} \ge q^{1/w}\varphi \to \varphi$. The shift rays do have $\rho = \varphi$, for every $k \ge 1$, by \cref{lem:shift}.
...
\Cref{thm:topedge} gives Lemma B on $(\beta',1]$ for every $\beta' > 0.6402121938$, hence on $(0.6402121938, 1]$ by taking a decreasing sequence of such $\beta'$ and noting that the exponent ranges nest. \Cref{def:hypL} gives it on $(0, 0.446717310462]$. The union leaves exactly the stated interval, on which nothing is claimed.
\end{proof}

\section{Reproducibility}
\label{sec:repro}

Everything numerical in this paper is re-checked by one script, \texttt{scripts/verify.py}, which uses the Python standard library only, takes no arguments, reads no files, and runs from the lane root in about two seconds on a laptop. Every assertion in it names the value obtained and the value expected, and any mismatch aborts the run. It covers exactly the following.

\begin{itemize}
  \item \emph{Constants.} $\log_3\varphi$, the edge $1/(2-\log_3\varphi) = 0.6402121938$, $c^* = 0.3597878$, the supergolden root $1.4655712319$ and the edge $0.6053028664$ it would give, the cap $2/(3+\log_3 5) = 0.447930988$, and, from $\lambda_8 = 456+3\sqrt{11017}$ and $\lambda_{10} = 6664.113662506$, the two ladder rungs $0.446717310462$ and $0.447597813454$ of \cref{fact:ladder}.
  \item \emph{The quarantined substitutions of \cref{rem:quarantine}.} $1/(2-\log_3 1.0639086) = 0.5145062$ and $1/(2-\log_3 1.0997454) = 0.5226147$, checked as arithmetic identities so that the two are never confused.
  \item \emph{\cref{lem:subsets}.} $D_k(w)$ against brute-force enumeration of all subsets for $w \le 14$ and $1 \le k \le 5$, and $D_k(w) \le \varphi^{\,w+k}$ for $1 \le k \le 6$, $w \le 60$.

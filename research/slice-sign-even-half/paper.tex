...
The diagonal slice is not generic in any dimension, and the side it misses on is decided by the parity of $D$ alone. We call this the \emph{sign law}. The odd half of it -- $\rho_D > f_D/3$ for every odd $D$, strictly whenever $D \not\equiv 1 \pmod 3$ -- was proved in the companion paper of this program \cite{slice-order}, where it falls out of a single pointwise inequality: at odd $D$ the design symbol is raised to an even power, so a certain trigonometric sum is nonnegative term by term and the census can only run ahead of $(f_D/q)^L$.
...
\medskip
\noindent\textbf{The results.} \Cref{thm:base5}: on the base-$5$ middle-digit design, $\rho_D < f_D/5$ for \emph{every} even $D \ge 2$. \Cref{thm:base3}: on the base-$3$ middle-digit design, $\rho_D < f_D/3$ for \emph{every} even $D \ge 2$. Combined with the odd half of \cite{slice-order}, the sign law now holds in sign at every dimension at base $3$ and at base $5$, with only one gap left anywhere: strictness in the exceptional odd residue classes.
...
We use \cref{prop:core} only in \cref{thm:reduction}(iii). The certificate theorems of \cref{sec:certs}, and therefore \cref{thm:base5} and \cref{thm:base3}, need none of it.
...
\]
and $B$ is decreasing on $D \ge 34$: the first factor visibly is, and the derivative of $(\log_5(4D)-1)/(D+\hat g_3)$ has the sign of $(D+\hat g_3)/(D\ln 5) - (\log_5(4D)-1)$, whose first term decreases to $1/\ln 5$ while the second increases, and which is $0.6943 - 2.0525 < 0$ at $D = 34$. With $B(34) \le 1.0089188$ it suffices that $h(D) = D\,r^{D-1} < \cos(\pi/4)/(8B(34)) = 0.0876070$; and $h$ decreases for $D > 1/\lvert\ln r\rvert = 3.7884$ with $h(34) \le 0.0056027$.
...
\]
This is a finite check for $38 \le D \le 400$: $182$ inequalities, each certified in interval arithmetic, with slack a factor $4.3\cdot 10^{42}$ at $D = 400$. Beyond $D = 400$, monotone domination closes it: $K_1(D+2) - 1 \le \tfrac{(D+2)(D+1)}{D(D-1)}\bigl(K_1(D)-1\bigr) + 2 \le 1.0104\,(K_1(D)-1)$, so the bound of \cref{prop:envelope3} has per-step ratio at most $1.0104 \cdot 0.7528157^2 \cdot 1.001 \le 0.573$, while the margin's per-step ratio is at least $\tfrac{D+2}{D+4}\bigl(1 - 3^{-K_1}\bigr) \ge 0.9950$; the pass at $D = 400$ with slack above $2$ therefore propagates to every larger even $D$, the accumulated $(1-3^{-K_1})$ factors multiplying to more than $1 - 10^{-29}$. Hence $(C_{K_1})$ holds and \cref{thm:column} gives $\rho_D < f_D/3$ for every even $D \ge 38$.
...
\begin{fact}
\label{fact:lstar}
$L^{*}(D) = L_0(D)$, the greatest odd integer $\le K^{*}(D)$, at all $24$ dimensions where $L^{*}$ is exactly known -- every even $D \le 120$ with the transient computed to exhaustion, plus $D = 38, 42, 44$ with interval enclosures of $K^{*}$ -- zero misses. This is a research-side computation; the bundled \texttt{check\_transient} pins $L^{*}$ itself at even $6 \le D \le 36$.
\end{fact}

\begin{fact}
\label{fact:transient}
At base $3$ and every even $6 \le D \le 36$, the minimal certificate depth is $K_{\min}(D) \in \{L^{*}(D)+1,\; L^{*}(D)+2\}$. Verified by \texttt{check\_transient} in \texttt{scripts/verify.py}. The same was checked by the same method at every exactly known $L^{*}$ through $D = 120$.
\end{fact}
...
The evidence is \cref{fact:transient} together with the proved bracket of \cref{thm:transient}, which already gives $K_{\min} \in [L^{*}+1, \lceil K^{*}\rceil + 2]$ for $D \ge 38$. The way it could fail is visible: the upper end of the proved bracket sits two steps above $\lceil K^{*}\rceil$ while $L^{*}$ sits at the greatest odd integer below $K^{*}$, so a dimension where $K^{*}$ falls just above an even integer could in principle need $L^{*}+3$. Nothing in the computed range does, and the two-leader model says nothing that forbids it.

\begin{remark}
\label{rem:notafit}
An earlier reading of the same data proposed $K_{\min} \approx (9/160)D^2$. That is refuted as an exact law -- the residual reaches $+9.78$ by $D = 178$ -- and the reason is now plain: $9/160 = 0.05625$ is the first two digits of $\tfrac14\log R = 0.0568486\ldots$. A numerical fit that agrees with a constant to two places is not a law.
\end{remark}
...
\]
is positive, negative, or zero, where $M_{\mathrm{even}}$ is the even core, of size $n = \lceil D/2 \rceil$. The sign of $\Delta_D$ is settled in \cref{thm:base3,thm:base5}. Whether $\Delta_D$ can be \emph{zero} is a different question, and it is a question about divisibility, not about size.
...
\begin{proof}
Combine \cref{b:cor:offclass} with \cref{b:fact:b3strict,b:fact:b5strict} through \cref{b:prop:2adic}, and use the non-strict inequality of \cref{thm:base3} on the odd side.
\end{proof}
...
\label{b:fact:fold}
The identity of \cref{b:prop:fold} holds as an exact integer identity, computed by fraction-free elimination, for the base-$3$ and base-$5$ designs at every $D \le 25$. Verified by \texttt{check\_fold} in \texttt{scripts/verify.py}; the same check was run to $D = 101$ at base $3$ during the research.
\end{fact}
...
\label{b:fact:tent}
$\operatorname{nullity}_2(M_{\mathrm{even}}^{(D)}) = \operatorname{tent}(D)$ at every odd $D \le 301$, computed from the matrix built directly from $P_D$ and not from any closed form. Verified by \texttt{check\_tent} in \texttt{scripts/verify.py}, which also runs the two finite windows of \cref{b:lem:M}; with the \texttt{--deep} flag the same check runs to $D \le 583$. The identity was checked to $D = 1401$ by the same method during the research, and the closed forms of \cref{b:lem:closed,b:lem:stepE} were checked against matrix-built brute force for odd $D \le 1401$ and against an independent minimisation of the generator-degree function for $R \le 2\cdot 10^{6}$.
\end{fact}
...
\]
attained at $D = 2J(k+1) + 3 + 2\bigl(J(k+2-2j) - 1\bigr)$, where the Smith profile is a flat block $a_i = j$ of large multiplicity plus a single spike of height $a_{\max}$. Verified at all $13$ pairs $(k,j)$ with $6 \le k \le 10$ and $1 \le j \le 4$ for which the maximiser lies in the scanned range. In addition, write $\mathcal{J} = J(k+2-2j)$ for that maximum. At the $i$-th dimension above such a site the profile is $\bigl[(j-1)^{p(i)},\, j^{\,q(i)},\, \text{spike}\bigr]$ with $p(i) = 2i-1$ (and $p(0) = 0$) and $q(i) = \mathcal{J} - i$, so $L_j = q(i)+1$: a plateau of length $2$ at $L_j = \mathcal{J}$, then unit decay, and the plateau lives in the site's own layer $j$. Verified on $5$ sites and $23$ rows. The maxima and the flank shape were checked by two implementations sharing no Smith routine; the $j = 2$ layer that anchors them is re-checked at every odd $D \le 151$ by \texttt{check\_layer2} in \texttt{scripts/verify.py}, and the deep rows are research-side computations by the same method.
\end{fact}

An earlier reading of the same data fitted the exponent $k - 1 - T(j-1)$ with $T$ the triangular numbers. That reading is false: it was fitted where $j = 2, 3$ makes the two formulas agree, and it dies at $j = 1$ and at $j = 4$, with explicit witnesses in octaves $9$ and $10$. The arithmetic exponent $k+2-2j$ is the correct one. Two rows in the corpus remain outliers: one spikeless maximiser and one two-tier profile whose middle tier sits one unit high; each has been re-derived by two independent routes and both stand as recorded data, not as counterexamples.
...
Six problems, in rough order of how much would fall. None of them is adjacent to any famous conjecture, and none of the mismatch or dimension-one machinery of the program's other lanes is used here or claimed here.
...
Every computational claim in this paper regenerates from two scripts in \texttt{scripts/}, run with plain \texttt{python3} from the paper's directory, standard library only, no dependencies. \texttt{verify.py} runs nine checks in about nine seconds and dies loudly on any mismatch; \texttt{verify.py --deep} extends the two largest domains and finishes in about twelve seconds; \texttt{figure.py} redraws both figures of this paper, as the SVG copies used by the README, in under a second.
...
\item \texttt{check\_transient}: base $3$, even $D = 6..36$ - $L^{*}(D)$ from the exact census and $K_{\min} \in \{L^{*}+1, L^{*}+2\}$, the shipped domain of \cref{fact:transient}; the run prints the full $(D, L^{*}, K_{\min})$ table.
...
\item \texttt{check\_strictness}: base $3$ class members $D \equiv 1 \pmod 3$, $13 \le D \le 151$, and base $5$ class members $D \equiv 1 \pmod 5$, $6 \le D \le 156$ - the valuation inequality of \cref{b:prop:2adic} by $2$-adic elimination at working precision $2^{1024}$ with a loud exactness guard, plus $v_2 \le n$ on the base-$3$ rows and, at the boundary case $D = 7$, the exact assertions $\Delta_7 = -148506048 = -2^6 \cdot 2320407$, $v_2(\Delta_7) = 6$, and $v_2(\det M_{\mathrm{even}}^{(7)}) = 7$.
\end{itemize}

One computation quoted in this paper is not bundled: the finite interval-arithmetic scan $(\star)$ in the proof of \cref{thm:base3}, $182$ inequalities at even $38 \le D \le 400$, together with the interval enclosures behind the constants of \cref{lem:sep5,lem:exit3,lem:subtree3,lem:cross3}. Those were carried out with rigorous outward-rounded interval arithmetic during the research; the proof states each constant and each domain exactly, so the scan is reproducible from the statements, but rerunning it needs an interval library and is not a standard-library computation. Beyond it, every number attributed to a named check regenerates from the two scripts above; the ranges each Fact declares research-side -- the sampled base-$5$ certificates to $D = 200$, the base-$3$ certificates to $D = 178$, the no-transient scan to $D \le 40$, the strictness and valuation scans to $D = 583$ and the deep isolated rows beyond, the layer-two and ceiling scans to $D = 4779$, and the octave scans of the cascade -- do not, and each is marked as research-side at its point of use.

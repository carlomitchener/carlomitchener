    ...
    for k, (label, removed) in enumerate(LAYERS):
        ...
        for i in range(1, 4):
            for j in range(1, 4):
                ...
                out.append('<rect x="%d" y="%d" width="%d" height="%d" fill="%s" stroke="%s" stroke-width="%.1f"/>' % (x, y, CELL, CELL, colour, MUTED, THIN))
        out.append(text(ox + panel // 2, oy + panel + 20, label, size=13, fill=MUTED, anchor="middle"))
    return out
...
def main():
    ...
    save(render(), os.path.join(target, "sponge.svg"))

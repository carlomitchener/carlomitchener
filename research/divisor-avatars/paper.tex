...
Say that $A = \sum_{w}A_{w}t^{w}$ is dominated by $B=\sum_{w}B_{w}t^{w}$, written $A \preceq B$, when $0 \le A_{w} \le B_{w}$ for every $w$. Domination is preserved by products of polynomials with nonnegative coefficients: if $A \preceq A'$ and $B \preceq B'$ then the coefficient of $t^{w}$ in $AB$ is $\sum_{j}A_{j}B_{w-j} \le \sum_{j}A'_{j}B'_{w-j}$, the coefficient of $t^{w}$ in $A'B'$.
...
(i) Since $x^{n} = \prod_{i}q_{i}^{a_{i}n}$ and the $q_{i}$ are distinct, $d(x^{n}) = \prod_{i=1}^{D}(a_{i}n+1)$. Write $b_{i}=a_{i}-1 \ge 0$. Then
\[
...
\[
  \prod_{i=1}^{D}(a_{i}n+1) = \prod_{i=1}^{D}\bigl(b_{i}n+(n+1)\bigr)
...
The Menger design has signature $(1,3,0,0)$, so \cref{lem:fill} gives
\[
...
Write $x = 2^{\alpha}3^{\beta}5^{\gamma}$ with $\alpha,\beta,\gamma \ge 1$; each of $30,60,120,180,240,360,900$ has this form. Then
\[
...
\item \emph{Robin corridors} (\cref{fact:robin}). The $140$ powers $x^{n}$ with $x \in \{30,60,120,180,240,360,900\}$ and $1 \le n \le 20$: $\sigma$ is computed exactly as an integer from the factorisation, the ratio in $40$-digit decimal arithmetic. The script asserts that the $140$ powers are $130$ distinct integers, that $131$ of the $140$ exceed $5040$ and are $122$ distinct integers, that all $131$ satisfy Robin's inequality, and that the maximum ratio is attained at $N=14400$ with value $1.573259905933$ to twelve places. It also checks the threshold constant $\exp\exp(15/(4e^{\gamma})) = 3681.17\ldots < 5040$.
...
The figure in the lane README is regenerated by \texttt{scripts/figure.py}, also plain \texttt{python3}, run as \texttt{python3 scripts/figure.py}; it writes \texttt{figures/sponge.svg} in well under a second and takes no input.

...
Two one-dimensional integrals do all the work. First, $\int_0^1 \sin(\pi a u)\, du = (1 - \cos \pi a)/(\pi a)$, which is $2/(\pi a)$ for odd $a$ and $0$ for even $a$. Second, expanding the square wave in its odd-harmonic series $s(x) = \frac4\pi \sum_{j \text{ odd}} \sin(\pi j x)/j$ (valid in $L^2$) and using $\int_0^1 \sin(\pi j n u)\sin(\pi a u)\, du = \tfrac12 \mathbf 1[jn = a]$ for positive integers $jn, a$,
\[
\int_0^1 s(nu)\,\sin(\pi a u)\, du = \frac{2}{\pi}\,\frac{n}{a}\,\mathbf 1\bigl[\,n \mid a \text{ and } a/n \text{ odd}\,\bigr].
\]
For odd $a$ and odd $n$ the cofactor condition is automatic. Now $\mathrm{C}_n = \tfrac14\bigl(1 - s_n(u)\bigr)\bigl(1 - s_n(v)\bigr)$ splits the double integral into four products of the two displays: for odd $a, b$,
\[
\int_0^1\!\!\int_0^1 \mathrm{C}_n \sin(\pi a u)\sin(\pi b v)\, du\, dv = \frac{1}{\pi^2 ab}\Bigl[ 1 - n\,\mathbf 1[n \mid a] - n\,\mathbf 1[n \mid b] + n^2\,\mathbf 1[n \mid \gcd(a,b)] \Bigr].
\]
...
\[
\sum_{a,b} \frac{\sigma_2(\gcd(a,b))}{(ab)^w} = \sum_{d} d^2 \sum_{d \mid a,\ d \mid b} (ab)^{-w} = \sum_d d^{2-2w} \Bigl( \sum_{a'} a'^{-w} \Bigr)^{\!2} = \lambda(2w-2)\, \lambda(w)^2,
\]
...
For $\operatorname{Re} s > 1$ the layer sum converges absolutely: $0 \le \mathrm{C}_n \le 1$. The interior coefficient of layer $n$ is $(\pi^2 ab)^{-1} n^2 \mathbf 1[n \mid \gcd(a,b)]$ exactly as in the proof of \cref{thm:spectrum}, so weighting by $n^{-s}$ and summing gives $(\pi^2 ab)^{-1} \sum_{n \mid \gcd(a,b)} n^{2-s}$ over odd $n$; for odd $a, b$ every divisor qualifies and the sum is finite, entire in $s$. The Dirichlet identity is the first display of the proof of \cref{thm:zetaquot} with $d^2$ replaced by $d^{2-s}$.
\end{proof}

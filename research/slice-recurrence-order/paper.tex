...
\emph{Step 1: the sheaf census is the free-end census.} Write $N_p = D(3^p-1)/2$, an integer for every $D$ because $3^p - 1$ is even. The coordinate sums $T \equiv N_L \pmod{3^L}$ are exactly $T = N_L - 3^L c'$ over $c' \in \mathbb{Z}$, where terms with $T$ outside $[0, D(3^L-1)]$ contribute nothing, so by Step 1 of the proof of \cref{thm:order},
\[
...
\emph{Step 3: the phases cancel.} By \cref{lem:circle}, $P_D(\zeta^{m3^j}) = e^{iD \cdot 2\pi m 3^j/3^L}\, \Phi_D\bigl(2\pi m 3^j/3^L\bigr)$, and
\[
...
\emph{Step 4: strictness off the residue class $D \equiv 1$.} The matrix $J = f_D I - 3 M_{\mathrm{even}}^{(D)}$ has integer entries, and $\chi_D(f_D/3) = \det\bigl( (f_D/3) I - M_{\mathrm{even}}^{(D)} \bigr) = 3^{-n_D} \det J$. Mod $3$ the term $3 M_{\mathrm{even}}^{(D)}$ vanishes, so $\det J \equiv f_D^{\,n_D} \pmod 3$. Since $f_D = 2^{D-1}(D+2)$, $3 \mid f_D$ exactly when $D \equiv 1 \pmod 3$; for $D \not\equiv 1 \pmod 3$, $\det J \not\equiv 0 \pmod 3$, so $\chi_D(f_D/3) \ne 0$ and $f_D/3$ is not an eigenvalue. Combined with $\rho_D \ge f_D/3$ and the fact that $\rho_D$ is an eigenvalue, $\rho_D > f_D/3$.
\end{proof}

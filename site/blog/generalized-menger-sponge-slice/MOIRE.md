...
The same trick works one dimension down. Here are the flat 2D carpets — the square grids from [How the pictures are made](index.md#how-the-pictures-are-made) — stacked the same way:
...
Is π lurking in here? What about the primes? We stacked, we measured, we proved — and the answers are stranger than the questions. Each headline links its paper on [the research shelf](https://github.com/carlomitchener/carlomitchener/tree/main/research), where every claim ships with a script that re-checks it.
...
![the correlation between parity carpets at odd scales 3 to 63, white where the scales are coprime; the all-white rows are the primes](files/extras/carpet-correlation.svg)

One honest footnote: this is a portrait of primality, not a shortcut to it — testing "is the correlation zero?" costs about as much as plain old trial division. The picture doesn't beat the arithmetic; it *is* the arithmetic, drawn. The paper: [Parity Carpets Correlate by gcd](https://github.com/carlomitchener/carlomitchener/tree/main/research/moire-correlation-laws).

**The snowflakes refuse to obey.** The diagonal cut smuggles a faint overtone into every snowflake — an extra wave vibrating twice as fast — and that overtone lets snowflake *m* whisper to snowflakes 2*m*−1 and 2*m*+1, its almost-doubles, even when the numbers share no factor. Coprime no longer means stranger; the slice carries gossip of its own. That write-up is still on the workbench.

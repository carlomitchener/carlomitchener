...
from mrlycloud.boto import put_png, s3_url
...
def prepare_printfiles(task: Task, gen: mrlygen.Gen) -> mrlygen.Gen:
    ...
    for pf in task.printfiles:
        ...
        file = mrlygen.File(width=grid_width, height=grid_height)
        gen.files.append(file)
    return gen

def prepare_tiles(task: Task, gen: mrlygen.Gen) -> mrlygen.Gen:
    sizes = [1, 3, 5, 7, 9]
    for size in sizes:
        file = mrlygen.File(width=size, height=size)
        gen.files.append(file)
    return gen

def process_printfiles(task: Task, gen: mrlygen.Gen, start: int) -> Task:
...
def process_tiles(task: Task, gen: mrlygen.Gen, start: int) -> Task:
    sizes = [1, 3, 5, 7, 9]
...
def mrly_generate(task: Task) -> Task:
    gen = mrlygen.Gen.from_dict(task.variation)
    seed(gen.seed)
    gen = prepare_printfiles(task, gen)
    gen = prepare_tiles(task, gen)
    gen = mrlygen.generate(gen)
    gen = mrlygen.render(gen, scale=UNIT_SCALE)

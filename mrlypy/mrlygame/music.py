...
import os
import mrlymusic
from mrlycore.state import choice, bool
from mrlymusic.enums import ChordType, Movement, Scale
from mrlymusic.models import Voice
...
def compose_music_params(way: Way) -> MusicParams:
    ...
    wave_type = choice([mrlymusic.WaveType.SINE, mrlymusic.WaveType.TRIANGLE])
...
def _compose(params: MusicParams, count: int):
    composer = mrlymusic.Composer(MUSIC_CONFIG)
    ...
    renderer = mrlymusic.Renderer(MUSIC_CONFIG, wave_type=params.wave_type, num_harmonics=params.num_harmonics)

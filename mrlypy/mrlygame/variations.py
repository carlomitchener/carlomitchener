...
from mrlytwo import Cell2d, carpet_2d
import mrlytile
from mrlytile import Tile, build, random_design, random_rotation
from mrlytile.enums import Design, Group
...
def setup_tile(task: Task) -> Task:
    ...
    tile_config = mrlytile.Config(
        min_size=MIN_TILE,
        max_size=MAX_TILE
    )
    tile = mrlytile.create(tile_config)
...
def basic_mask(min_size: int, max_size: int) -> Tile:
    tile_config = mrlytile.Config(groups=[Group.GENERAL, Group.FRACTAL, Group.MAGIC], min_size=min_size, max_size=max_size)
    tile = mrlytile.create(tile_config)
    tile = build(tile)
    return tile

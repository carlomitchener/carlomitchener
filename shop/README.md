...
- COMPLETE `complete.py` - archives the task to `data/automator/tasks/<key>.json` and clears `task.json`; ARCHIVE reruns it if a tick died between the two. It never wakes the site: the site ignores products whose batch is unreleased.
...
- The site takes the primary and the design from the handle, reads group and secondary from the archived task, and the release date from `batches/` (`scripts/snapshot.ts`). The two products of a design render as one page at `/{slug}/{design}/`: the default follows the site mode, a pill swaps them, and every grid shows the sibling matching the mode.

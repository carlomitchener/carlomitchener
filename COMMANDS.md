...
- `bun run fake` - the same two files, invented, so the site builds with no shop and no posts.
...
- `bun run dev` - build once, then serve `dist/` on port 3000, with `/cdn/feed/` served from `data/carlomitchener/feed/`.

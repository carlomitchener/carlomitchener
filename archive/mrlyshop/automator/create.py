...
from mrlypaint.colors import get_primary_inks
...
def create_variation(task: Task) -> Task:
    ...
    config = mrlygen.Config(
        tile=mrlytile.Config(min_size=3, max_size=9, anti=False),
        paint=mrlypaint.Config(primaries=primaries),
        files=[],
    )
    gen = mrlygen.create(config)
    gen.key = task.key
    task.seed = gen.seed

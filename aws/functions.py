...
def deploy(source_dir: str, function_suffix: str, config: dict):
    ...
    for package in PACKAGES:
        dest = os.path.join(source_dir, package)
        print(f"Copying {package}...")
        shutil.copytree(os.path.join(ROOT, package), dest, ignore=shutil.ignore_patterns("__pycache__"))
...
def automator():
    deploy(os.path.join(ROOT, "shop", "automator"), "mrlyshop-automator", MRLYSHOP_AUTOMATOR_CONFIG)

def storefront():
    deploy(os.path.join(ROOT, "shop", "storefront"), "mrlyshop-storefront", MRLYSHOP_STOREFRONT_CONFIG)
